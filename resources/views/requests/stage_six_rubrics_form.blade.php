...
<body class="h-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 italic-arabic {{ !$isEditMode ? 'view-mode' : '' }}">
          ...
          <div class="flex items-center gap-4">
             ...
             <div>
                <div class="flex items-center gap-3 mb-1">
                  ...
                  <h2 class="text-2xl font-bold text-slate-800 dark:text-white">مقاييس تقييم البرنامج (الروبريك)</h2>
                </div>
                <p class="text-slate-700 dark:text-slate-300 mr-5 font-medium">التقييم وفق معايير الاعتماد - المرحلة السادسة</p>
             </div>
          </div>
  ...
  <script>
    // Server data
    window.SAVE_URL = "{{ route('requests.stage_six.rubrics_save', $accreditationRequest) }}";
