...
<body class="bg-(--bg-main) text-(--text-primary) min-h-screen font-sans">
    ...
    <div class="max-w-6xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <div class="rounded-2xl border border-(--border-primary) bg-(--surface-card) shadow-sm overflow-hidden">
            <div class="px-6 py-5 flex items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    <div
                        class="w-12 h-12 rounded-2xl bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400 flex items-center justify-center border border-brand-100 dark:border-brand-500/20 shadow-inner shrink-0">
                        <i class="fa-solid fa-file-lines text-xl"></i>
                    </div>
                    <div>
                        <h1 class="text-lg md:text-xl font-bold text-(--text-primary)">نظام إدارة بيانات البرنامج
                            الأكاديمي</h1>
                        <p class="text-xs text-(--text-secondary) mt-0.5">تعبئة البيانات الأساسية للمرحلة الثانية من طلب
                            الاعتماد</p>
                    </div>
                </div>
        ...
        <div class="rounded-2xl border border-(--border-primary) bg-(--surface-card) shadow-sm overflow-hidden">
            ...
            {{-- ═══════ TAB 1: القرارات ═══════ --}}
            <div x-show="currentTab === 0" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="p-6 space-y-6">
                <div class="flex items-center gap-3 mb-2">
                    <div
                        class="w-9 h-9 rounded-xl bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 flex items-center justify-center border border-blue-100 dark:border-blue-500/20 shrink-0">
                                ...
                                <tr class="hover:bg-(--bg-main)/50 transition-colors">
                                    ...
                                    <td class="px-4 py-3">
                                        <div id="file_container_{{ $i }}">
                                            ...
                                            <div id="file_preview_{{ $i }}"
                                                class="{{ !empty($files[$i]) ? 'flex' : 'hidden' }} items-center gap-2 flex-wrap">
                                                <a id="file_view_btn_{{ $i }}"
                                                    href="{{ !empty($files[$i]) ? route('requests.stage_two.view_file', ['accreditationRequest' => $accreditationRequest->id, 'formSubmission' => $formSubmission->id, 'decisionIndex' => $i]) : '#' }}"
                                                    target="_blank"
                                                ...
                                                <button type="button"
                                                    @click="confirmRemoveFile({{ $i }})" data-action-area
                                                ...
                                                @if (!empty($files[$i]))
                                                    <input type="hidden" id="existing_file_path_{{ $i }}"
                                                        value="{{ $files[$i] }}">
                                                @endif
                                            </div>

                                            {{-- File Upload Input --}}
                                            <input type="file" id="decision_file_{{ $i }}"
                                                accept="application/pdf" data-action-area
                ...
                <div class="rounded-xl border border-(--border-primary) bg-(--surface-card) p-5 space-y-4">
                    ...
                    <button type="button" onclick="addNoteRow('decisionsNotesContainer', 'decisions')"
                        data-action-area
            ...
            {{-- TAB 2: Student Stats --}}
            <div x-show="currentTab === 1" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="p-6 space-y-6">
                <div class="flex items-center gap-3 mb-2">
                    <div
                        class="w-9 h-9 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center border border-emerald-100 dark:border-emerald-500/20 shrink-0">
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-(--border-primary)">
                            @php
                                $studentSections = [
                                    'planned' => [
                                        'label' => 'عدد الطلبة المخطط التحاقهم بالبرنامج',
                                        'rows' => [
                                            'general' => 'قبول عام',
                                            'special' => 'قبول خاص',
                                    ...
                                    'total' => [
                                        'label' => 'العدد الكلي للطلاب الملتحقين بالبرنامج',
                                        'rows' => [
                                            'general' => 'قبول عام',
                                            'special' => 'قبول خاص',
                                            'international' => 'قبول دولي',
                                    ...
                                    'average' => [
                                        'label' => 'متوسط عدد الطلبة في الشعبة الدراسية',
                                        'rows' => [
                                            'male' => 'ذكور',
                                            'female' => 'إناث',
                                        ],
                                    ...
                                    'graduates_higher_ed' => [
                                        'label' => 'عدد الخريجين الذين يواصلون تعليمهم في الدراسات العليا',
                                        'rows' => [
                                            'male' => 'ذكور',
                                            'female' => 'إناث',
                                        ],
                                    ...
                                    'graduates_employed' => [
                                        'label' => 'عدد الخريجين الذين التحقوا بوظائف',
                                        'rows' => [
                                            'male' => 'ذكور',
                                            'female' => 'إناث',
                                        ],
                            ...
                            @foreach ($studentSections as $sectionKey => $section)
                                ...
                                @foreach ($section['rows'] as $rowKey => $rowLabel)
                                    <tr class="hover:bg-(--bg-main)/50 transition-colors">
                                        @if ($loop->first)
                                            <td rowspan="{{ $rowCount }}"
                                                class="px-4 py-3 font-bold text-(--text-primary) border-e border-(--border-primary) bg-(--bg-main)/30 align-top">
                                                {{ $section['label'] }}</td>
                                        @endif
                                        ...
                                        @foreach (['past', 'current', 'next'] as $period)
                                            <td class="px-4 py-3">
                                                <input type="number" min="0"
                                                    data-student="{{ $sectionKey }}"
                                                    data-row="{{ $rowKey }}" data-period="{{ $period }}"
                                        ...
                                        @foreach (['past', 'current', 'next'] as $period)
                                            <td class="px-4 py-3">
                                                <input type="text" readonly
                                                    data-student-total="{{ $sectionKey }}"
                                                    data-period="{{ $period }}" value="0"
                ...
                <div class="rounded-xl border border-(--border-primary) bg-(--surface-card) p-5 space-y-4">
                    ...
                    <button type="button" onclick="addNoteRow('studentsNotesContainer', 'students')" data-action-area
                        class="inline-flex items-center gap-2 text-xs font-bold text-(--text-secondary) hover:text-(--text-primary) transition-colors cursor-pointer px-1"><i
                            class="fa-solid fa-plus-circle"></i> إضافة ملاحظة</button>
                </div>
            </div>

            {{-- TAB 3: Faculty Stats --}}
            <div x-show="currentTab === 2" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="p-6 space-y-6">
                <div class="flex items-center gap-3 mb-2">
                    <div
                        class="w-9 h-9 rounded-xl bg-violet-50 dark:bg-violet-500/10 text-violet-600 dark:text-violet-400 flex items-center justify-center border border-violet-100 dark:border-violet-500/20 shrink-0">
                    ...
                    <table class="w-full text-sm">
                                ...
                                <tr class="hover:bg-(--bg-main)/50 transition-colors">
                                    ...
                                    @foreach (['male', 'female', 'load', 'parttime'] as $col)
                                        <td class="px-4 py-3">
                                            <input type="number" min="0"
                                                {{ $col === 'load' ? 'step=0.1' : '' }}
                                                data-faculty="{{ $rankKey }}" data-col="{{ $col }}"
                                                class="w-full bg-(--bg-main) border border-(--border-primary) text-(--text-primary) text-sm rounded-lg px-3 py-2 text-center focus:outline-none focus:ring-2 focus:ring-brand-500/40 transition-all"
                        ...
                        <tfoot>
                            <tr class="bg-(--bg-main)/50 font-bold">
                                ...
                                @foreach (['male', 'female', 'load', 'parttime'] as $col)
                                    <td class="px-4 py-3">
                                        <input type="text" readonly data-faculty-total="{{ $col }}"
                                            value="0"
                                            class="w-full bg-(--border-primary)/30 border border-(--border-primary) text-(--text-primary) text-sm rounded-lg px-3 py-2 text-center font-bold">
                                    </td>
                                @endforeach
                ...
                <div class="rounded-xl border border-(--border-primary) bg-(--surface-card) p-5 space-y-4">
                    ...
                    <button type="button" onclick="addNoteRow('facultyNotesContainer', 'faculty')" data-action-area
                        class="inline-flex items-center gap-2 text-xs font-bold text-(--text-secondary) hover:text-(--text-primary) transition-colors cursor-pointer px-1"><i
                            class="fa-solid fa-plus-circle"></i> إضافة ملاحظة</button>
                </div>
            </div>
            ...
            <div x-show="currentTab === 3" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="p-6 space-y-6">
                <div class="flex items-center gap-3 mb-2">
                    <div
                        class="w-9 h-9 rounded-xl bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 flex items-center justify-center border border-amber-100 dark:border-amber-500/20 shrink-0">
                    ...
                    <table class="w-full text-sm" id="staffTable">
                        ...
                        <tbody class="divide-y divide-(--border-primary)" id="staffTableBody">
                            <tr data-staff-id="1" class="hover:bg-(--bg-main)/50 transition-colors">
                                <td class="px-4 py-3"><input type="text" placeholder="أدخل الاسم"
                                        class="staff-name w-full bg-(--bg-main) border border-(--border-primary) text-(--text-primary) text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500/40 transition-all placeholder:text-(--text-secondary)/50"
                                        @input="markChanged()"></td>
                                <td class="px-4 py-3">
                                    <select
                                        class="staff-degree w-full bg-(--bg-main) border border-(--border-primary) text-(--text-primary) text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500/40 transition-all"
                                        @input="markChanged()">
                                ...
                                <td class="px-4 py-3"><input type="text" placeholder="التخصص العام"
                                        class="staff-major w-full bg-(--bg-main) border border-(--border-primary) text-(--text-primary) text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500/40 transition-all placeholder:text-(--text-secondary)/50"
                                        @input="markChanged()"></td>
                                <td class="px-4 py-3"><input type="text" placeholder="التخصص الدقيق"
                                        class="staff-minor w-full bg-(--bg-main) border border-(--border-primary) text-(--text-primary) text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500/40 transition-all placeholder:text-(--text-secondary)/50"
                                        @input="markChanged()"></td>
                                <td class="px-4 py-3"><input type="text" placeholder="بلد التخرج"
                                        class="staff-country w-full bg-(--bg-main) border border-(--border-primary) text-(--text-primary) text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500/40 transition-all placeholder:text-(--text-secondary)/50"
                                        @input="markChanged()"></td>
                                <td class="px-4 py-3"><input type="number" placeholder="سنة" min="1900"
                                        max="2100"
                                        class="staff-year w-full bg-(--bg-main) border border-(--border-primary) text-(--text-primary) text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500/40 transition-all"
                                        @input="markChanged()"></td>
                                <td class="px-4 py-3 text-center" data-action-area>
                                    <button type="button" onclick="deleteStaffRow(this)"
                                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-50 text-red-700 border border-red-200 hover:bg-red-100 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20 text-xs font-bold transition-colors cursor-pointer"
                                        data-action-area><i class="fa-solid fa-trash-can"></i> حذف</button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <button type="button" onclick="addStaffRow()" data-action-area
                    class="inline-flex items-center gap-2 text-xs font-bold text-(--text-secondary) hover:text-(--text-primary) transition-colors cursor-pointer px-1"><i
                        class="fa-solid fa-plus-circle"></i> إضافة عضو جديد</button>
            </div>
    ...
    <div x-show="showExitModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto">
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity" @click="showExitModal = false">
        </div>
        <div class="flex min-h-full items-center justify-center p-4 text-center">
            <div x-show="showExitModal" x-transition:enter="ease-out duration-300"
                ...
                <div
                    class="w-12 h-12 rounded-full bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center mx-auto mb-4 border border-amber-100 dark:border-amber-500/20">
                ...
                <p class="text-sm text-center text-(--text-secondary) mb-6">هل ترغب في حفظ التغييرات قبل المغادرة؟ إذا
                    غادرت دون حفظ ستفقد التعديلات الأخيرة.</p>

                <div class="flex flex-col sm:flex-row items-center justify-center gap-3">
                    <button type="button"
                        @click="showExitModal = false; submitFormData('{{ route('requests.show', $accreditationRequest) }}')"
    ...
    <div x-show="showDeleteModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto">
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity" @click="showDeleteModal = false">
        </div>
        <div class="flex min-h-full items-center justify-center p-4 text-center">
            <div x-show="showDeleteModal" x-transition:enter="ease-out duration-300"
                ...
                <div
                    class="w-12 h-12 rounded-full bg-red-50 dark:bg-red-500/10 flex items-center justify-center mx-auto mb-4 border border-red-100 dark:border-red-500/20">
                ...
                <p class="text-sm text-center text-(--text-secondary) mb-6">هل أنت متأكد من حذف هذا المرفق؟ سيتم حذف
                    الملف نهائياً ولا يمكن التراجع عن هذا الإجراء بعد حفظ التغييرات.</p>
    ...
    <script>
        ...
        const INPUT_CLASS =
            'w-full bg-(--bg-main) border border-(--border-primary) text-(--text-primary) text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500/40 transition-all placeholder:text-(--text-secondary)/50';
        ...
        function stageTwoApp() {
            return {
                ...
                tabs: [{
                        ...
                        activeClasses: 'border-blue-600 text-blue-600 dark:text-blue-400 dark:border-blue-400'
                    },
                    {
                        label: 'إحصائيات الطلاب',
                        icon: 'fa-solid fa-chart-bar',
                        activeClasses: 'border-emerald-600 text-emerald-600 dark:text-emerald-400 dark:border-emerald-400'
                    },
                    {
                        label: 'إحصائيات الهيئة',
                        icon: 'fa-solid fa-chalkboard-user',
                        activeClasses: 'border-violet-600 text-violet-600 dark:text-violet-400 dark:border-violet-400'
                    },
                    {
                        label: 'بيانات الهيئة',
                        icon: 'fa-solid fa-users',
                        activeClasses: 'border-amber-600 text-amber-600 dark:text-amber-400 dark:border-amber-400'
                    },
                ],
                ...
                updateProgress() {
                    const fields = document.querySelectorAll(
                        'input:not([type="hidden"]):not([type="file"]):not([readonly]), select');
                    let filled = 0;
                    fields.forEach(f => {
                ...
                calculateStudentTotal(section) {
                    ['past', 'current', 'next'].forEach(period => {
                        const inputs = document.querySelectorAll(
                            `[data-student="${section}"][data-period="${period}"]`);
                        let sum = 0;
                        inputs.forEach(inp => {
                            sum += Math.max(0, Number(inp.value) || 0);
                        });
                        const totalField = document.querySelector(
                            `[data-student-total="${section}"][data-period="${period}"]`);
                ...
                async discardAndLeave() {
                    ...
                    if (tempPaths.length > 0) {
                        ...
                        try {
                            ...
                            await fetch("{{ route('temp_files.cleanup') }}", {
                                method: 'POST',
                                body: formData
                            });
                        } catch (e) {}
                ...
                async submitFormData(redirectUrl = null) {
                    ...
                    for (let i = 1; i <= 8; i++) {
                        ...
                        if (fileInput && fileInput.files.length > 0) payload.append('decision_file_' + i, fileInput
                            .files[0]);
                    }
                    ...
                    try {
                        const response = await fetch(
                            "{{ route('requests.stage_two.save', [$accreditationRequest, $formSubmission]) }}", {
                                method: 'POST',
                                body: payload,
                                headers: {
                        ...
                        if (result.success) {
                            ...
                            const targetUrl = redirectUrl ||
                                "{{ route('requests.stage', [$accreditationRequest, 'stage_two']) }}";
                            setTimeout(() => {
                ...
                resetForm() {
                    ...
                    document.querySelectorAll(
                        'input:not([readonly]):not([type="hidden"]):not([type="file"]), select, textarea').forEach(f =>
                        f.value = '');
                ...
                loadFromServer() {
                    const saved = {!! isset($formSubmission->form_data) && $formSubmission->form_data
                        ? json_encode($formSubmission->form_data, JSON_UNESCAPED_UNICODE)
                        : 'null' !!};
                    if (!saved) return;
                    ...
                    try {
                        ...
                        if (saved.decisions) {
                            saved.decisions.forEach(d => {
                                ...
                                `[data-decision="${d.id}"][data-field="number"]`);
                                const auth = document.querySelector(
                                    `[data-decision="${d.id}"][data-field="authority"]`);
                        ...
                        ['decisionsNotesContainer', 'studentsNotesContainer', 'facultyNotesContainer'].forEach(c =>
                            updateNoteNumbers(c));
                ...
                init() {
                    ...
                    @if (isset($readonly) && $readonly)
                        this.$nextTick(() => {
                            document.querySelectorAll('input, select, textarea, button:not([data-keep])').forEach(
                                el => el.disabled = true);
                            document.querySelectorAll('[data-action-area]').forEach(el => el.style.display =
                            'none');
                        });
                    @endif
                }
        ...
        function addStaffRow() {
            const tbody = document.getElementById('staffTableBody');
            const newId = Math.max(...Array.from(tbody.querySelectorAll('tr[data-staff-id]')).map(tr => parseInt(tr.dataset
                .staffId) || 0), 0) + 1;
            const tr = document.createElement('tr');
        ...
        function updateNoteNumbers(containerId) {
            ...
            if (rows.length === 0) {
                container.innerHTML =
                    `<div class="no-notes-placeholder text-sm text-(--text-secondary) italic px-4 py-3 bg-(--bg-main) rounded-lg border border-dashed border-(--border-primary) text-center"><i class="fa-solid fa-circle-info me-1"></i> لا توجد ملاحظات</div>`;
            } else {
                const placeholder = container.querySelector('.no-notes-placeholder');
                if (placeholder) placeholder.remove();
        ...
        async function handleFileSelect(event, id) {
            ...
            const url =
                "{{ route('requests.stage_two.upload_file', ['accreditationRequest' => $accreditationRequest->id, 'formSubmission' => $formSubmission->id, 'decisionIndex' => ':id']) }}"
                .replace(':id', id);
        ...
        function showNotification(message, type = 'info') {
            ...
            el.className =
                `${colors[type] || colors.info} text-white px-5 py-3 rounded-xl shadow-lg text-sm font-bold flex items-center gap-2 animate-[slideIn_0.3s_ease-out]`;
            const icons = {
