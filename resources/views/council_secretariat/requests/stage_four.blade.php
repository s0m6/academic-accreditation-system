...
@php
    $breadcrumbs = [
        ...
        'طلبات الاعتماد' => '#',
        'اختيار لجنة التقييم' => route('council_secretariat.requests.stage_four')
    ];
@endphp

@section('title', 'طلبات الاعتماد - اختيار لجنة التقييم')
@section('title2', 'إدارة طلبات المرحلة الرابعة')
@section('description', 'عرض ومراجعة طلبات الاعتماد في مرحلة اختيار وتشكيل لجنة التقييم')
    ...
    <div class="shadow-md rounded-2xl overflow-hidden border border-(--border-primary) bg-(--surface-card)">
        ...
        <div class="border-b border-(--border-primary) bg-(--content-container) px-6 py-5 flex items-center justify-between">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 shrink-0 rounded-full bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 flex justify-center items-center shadow-inner border border-indigo-100 dark:border-indigo-500/20">
                    <span class="text-2xl font-black">4</span>
                </div>
                <div>
                    <h3 class="font-bold text-(--text-primary) text-lg">قائمة اختيار لجنة التقييم</h3>
                    <p class="text-xs md:text-sm text-(--text-secondary) mt-0.5">متابعة تشكيل لجان التقييم الخارجى للبرامج الأكاديمية</p>
                </div>
            </div>
        </div>
                            ...
                            <td colspan="4" class="px-6 py-12 text-center text-(--text-secondary) border-t border-(--border-primary)">
                                <div class="flex flex-col items-center justify-center space-y-4">
                                    <div class="w-16 h-16 rounded-full bg-(--bg-main) flex items-center justify-center">
                                        <i class="fa-solid fa-inbox text-2xl text-(--text-secondary) dark:text-gray-500"></i>
                                    </div>
                                    <span class="text-base md:text-lg font-bold">لا توجد طلبات في مرحلة اختيار لجنة التقييم حالياً</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
