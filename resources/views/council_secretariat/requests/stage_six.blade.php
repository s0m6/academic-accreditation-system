...
@php
    $breadcrumbs = [
        ...
        'طلبات الاعتماد' => '#',
        'تقارير نتائج التقييم(الأولية)' => route('council_secretariat.requests.stage_six')
    ];
@endphp

@section('title', 'طلبات الاعتماد - تقارير نتائج التقييم(الأولية)')
@section('title2', 'إدارة طلبات المرحلة السادسة')
@section('description', 'عرض ومراجعة تقارير نتائج التقييم الأولية المقدمة من لجان التقييم')
    ...
    <div class="shadow-md rounded-2xl overflow-hidden border border-(--border-primary) bg-(--surface-card)">
        ...
        <div class="border-b border-(--border-primary) bg-(--content-container) px-6 py-5 flex items-center justify-between">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 shrink-0 rounded-full bg-cyan-50 dark:bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 flex justify-center items-center shadow-inner border border-cyan-100 dark:border-cyan-500/20">
                    <span class="text-2xl font-black">6</span>
                </div>
                <div>
                    <h3 class="font-bold text-(--text-primary) text-lg">قائمة تقارير نتائج التقييم الأولية</h3>
                    <p class="text-xs md:text-sm text-(--text-secondary) mt-0.5">مراجعة النتائج الأولية لعملية التقييم البرامجي</p>
                </div>
            </div>
        </div>
                            ...
                            <td colspan="4" class="px-6 py-12 text-center text-(--text-secondary) border-t border-(--border-primary)">
                                <div class="flex flex-col items-center justify-center space-y-4">
                                    <div class="w-16 h-16 rounded-full bg-(--bg-main) flex items-center justify-center">
                                        <i class="fa-solid fa-file-waveform text-2xl text-(--text-secondary) dark:text-gray-500"></i>
                                    </div>
                                    <span class="text-base md:text-lg font-bold">لا توجد طلبات في مرحلة تقارير النتائج الأولية حالياً</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
