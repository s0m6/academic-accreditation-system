...
@php
    $breadcrumbs = [
        ...
        'طلبات الاعتماد' => '#',
        'توصيات اللجنة والرد عليها' => route('council_secretariat.requests.stage_seven')
    ];
@endphp

@section('title', 'طلبات الاعتماد - توصيات اللجنة والرد عليها')
@section('title2', 'إدارة طلبات المرحلة الخامسة')
@section('description', 'عرض ومراجعة توصيات اللجنة وردود المؤسسات التعليمية عليها')
    ...
    <div class="shadow-md rounded-2xl overflow-hidden border border-(--border-primary) bg-(--surface-card)">
        ...
        <div class="border-b border-(--border-primary) bg-(--content-container) px-6 py-5 flex items-center justify-between">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 shrink-0 rounded-full bg-teal-50 dark:bg-teal-500/10 text-teal-600 dark:text-teal-400 flex justify-center items-center shadow-inner border border-teal-100 dark:border-teal-500/20">
                    <span class="text-2xl font-black">7</span>
                </div>
                <div>
                    <h3 class="font-bold text-(--text-primary) text-lg">قائمة توصيات اللجنة والرد عليها</h3>
                    <p class="text-xs md:text-sm text-(--text-secondary) mt-0.5">متابعة التوصيات الفنية والردود الرسمية بين اللجنة والبرنامج</p>
                </div>
            </div>
        </div>
                            ...
                            <td colspan="4" class="px-6 py-12 text-center text-(--text-secondary) border-t border-(--border-primary)">
                                <div class="flex flex-col items-center justify-center space-y-4">
                                    <div class="w-16 h-16 rounded-full bg-(--bg-main) flex items-center justify-center">
                                        <i class="fa-solid fa-comments text-2xl text-(--text-secondary) dark:text-gray-500"></i>
                                    </div>
                                    <span class="text-base md:text-lg font-bold">لا توجد طلبات في مرحلة توصيات اللجنة حالياً</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
