...
@php
    $breadcrumbs = [
        ...
        'طلبات الاعتماد' => '#',
        'تحديد جدول الزيارة' => route('council_secretariat.requests.stage_five')
    ];
@endphp

@section('title', 'طلبات الاعتماد - تحديد جدول الزيارة')
@section('title2', 'إدارة طلبات المرحلة الخامسة')
@section('description', 'عرض ومراجعة طلبات الاعتماد في مرحلة تحديد جدول الزيارة الميدانية')
    ...
    <div class="shadow-md rounded-2xl overflow-hidden border border-(--border-primary) bg-(--surface-card)">
        ...
        <div class="border-b border-(--border-primary) bg-(--content-container) px-6 py-5 flex items-center justify-between">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 shrink-0 rounded-full bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 flex justify-center items-center shadow-inner border border-blue-100 dark:border-blue-500/20">
                    <span class="text-2xl font-black">5</span>
                </div>
                <div>
                    <h3 class="font-bold text-(--text-primary) text-lg">قائمة تحديد جدول الزيارة</h3>
                    <p class="text-xs md:text-sm text-(--text-secondary) mt-0.5">تنسيق ومتابعة مواعيد الزيارات الميدانية للجان التقييم</p>
                </div>
            </div>
        </div>
                            ...
                            <td colspan="4" class="px-6 py-12 text-center text-(--text-secondary) border-t border-(--border-primary)">
                                <div class="flex flex-col items-center justify-center space-y-4">
                                    <div class="w-16 h-16 rounded-full bg-(--bg-main) flex items-center justify-center">
                                        <i class="fa-solid fa-calendar-days text-2xl text-(--text-secondary) dark:text-gray-500"></i>
                                    </div>
                                    <span class="text-base md:text-lg font-bold">لا توجد طلبات في مرحلة تحديد جدول الزيارة حالياً</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
